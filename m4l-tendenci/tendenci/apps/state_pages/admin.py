...
from tendenci.apps.state_pages.forms import StatePageAdminForm
...
class StatePageAdmin(admin.ModelAdmin):
    ...
    def edit_link(self, obj):
        link = '<a href="%s" title="edit">Edit</a>' % reverse('admin:state_pages_statepage_change', args=[obj.pk])
        return link
    ...
    def view_on_site(self, obj):
        ...
        link = '<a href="%s" title="%s"><img src="%s" alt="%s" title="%s" /></a>' % (
            reverse('state_page', args=[obj.slug]),
            obj.title,
            link_icon,
            obj.title,
            obj.title
    ...
    def log_deletion(self, request, object, object_repr):
        super(StatePageAdmin, self).log_deletion(request, object, object_repr)
        log_defaults = {
            'event_id' : 583000,
    ...
    def log_change(self, request, object, message):
        super(StatePageAdmin, self).log_change(request, object, message)
        log_defaults = {
            'event_id' : 582000,
    ...
    def log_addition(self, request, object, message):
        super(StatePageAdmin, self).log_addition(request, object, message)
        log_defaults = {
            'event_id' : 581000,
...
admin.site.register(StatePage, StatePageAdmin)
