...
class LatestEntriesFeed(SubFeed):
    ...
    def items(self):
        items = ChapterPage.objects.filter(**PUBLIC_FILTER).filter(syndicate=True).order_by('-create_dt')[:20]
        return items
...
class ChapterPageSitemap(TendenciSitemap):
    """ Sitemap information for pages """
    changefreq = "yearly"
    priority = 0.6

    def items(self):
        items = ChapterPage.objects.filter(**PUBLIC_FILTER).order_by('-create_dt')
        return items
