...
CONTRIBUTOR_CHOICES = (
    (ChapterPage.CONTRIBUTOR_AUTHOR, mark_safe('Author <i class="gauthor-info fa fa-lg fa-question-circle"></i>')),
    (ChapterPage.CONTRIBUTOR_PUBLISHER, mark_safe('Publisher <i class="gpub-info fa fa-lg fa-question-circle"></i>'))
)
...
class ChapterPageAdminForm(TendenciBaseForm):
    content = forms.CharField(required=False,
        widget=TinyMCE(attrs={'style':'width:100%'},
        mce_attrs={'storme_app_label':ChapterPage._meta.app_label,
        'storme_model':ChapterPage._meta.model_name.lower()}))
    ...
    def __init__(self, *args, **kwargs):
        super(ChapterPageAdminForm, self).__init__(*args, **kwargs)
        if self.instance.pk:
    ...
    def clean(self):
        cleaned_data = super(ChapterPageAdminForm, self).clean()
        ...
        if self.instance:
            ...
            if ChapterPage.objects.filter(slug=slug).exclude(guid=guid).exists():
                self._errors['slug'] = self.error_class([_('Duplicate value for slug.')])
                del cleaned_data['slug']
        # Case 2: Add new Page
        else:
            if ChapterPage.objects.filter(slug=slug).exists():
                self._errors['slug'] = self.error_class([_('Duplicate value for slug.')])
                del cleaned_data['slug']
...
class ChapterPageForm(TendenciBaseForm):
    ...
    contributor_type = forms.ChoiceField(choices=CONTRIBUTOR_CHOICES,
                                         initial=ChapterPage.CONTRIBUTOR_AUTHOR,
    ...
    chapter = forms.ModelChoiceField(queryset=Chapter.objects, widget=forms.HiddenInput)
    ...
    def __init__(self, *args, **kwargs):
        super(ChapterPageForm, self).__init__(*args, **kwargs)
    ...
    def clean(self):
        cleaned_data = super(ChapterPageForm, self).clean()
        slug = cleaned_data.get('slug')
    ...
    def save(self, *args, **kwargs):
        page = super(ChapterPageForm, self).save(*args, **kwargs)
