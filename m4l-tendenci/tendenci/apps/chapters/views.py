...
@is_enabled('chapters')
# @login_required this is commented so it can be used in ajax request for national subs
def chapter_membership_add(request, chapter_id=0, app_type='chapter',
                           template='chapters/applications/add.html', **kwargs):
    """
    Chapter membership application form.
    """
    
    if not is_ajax(request) and request.user.is_anonymous:
